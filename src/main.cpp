...
FrameInfo LoadFrameInfo(const filesystem::path &inputDir, const int &idx) {
    ...
    //打印beauty的值 beauty中的颜色值已进行了归一化处理
    // for(int i=0;i<beauty.m_height;i++){
    //     for(int j=0;j<beauty.m_width;j++){
    //         std::cout<<beauty(j,i)<<std::endl;
    //     }
    // }
    //打印beauty的shape
    std::cout<<"beauty shape:"<<beauty.m_width<<" "<<beauty.m_height<<std::endl;
    return frameInfo;
}
